...
public class ChipRefineryScreen extends HandledScreen<ChipRefineryScreenHandler> {
    ...
    public ChipRefineryScreen(ChipRefineryScreenHandler handler, PlayerInventory inventory, Text title) {
    ...
    protected void drawMouseoverTooltip(MatrixStack matrices, int x, int y) {
        ...
        if (this.canCraft) {
            ...
            for (int l = this.scrollOffset; l < k && l < ((ChipRefineryScreenHandler)this.handler).getAvailableRecipeCount(); ++l) {
                ...
                renderTooltip(matrices, list.get(l).getOutput(this.client.world.getRegistryManager()), x, y);            }
        }
    }
    ...
    private void renderRecipeIcons(MatrixStack matrices, int x, int y, int scrollOffset) {
        ...
        for(int i = this.scrollOffset; i < scrollOffset && i < ((ChipRefineryScreenHandler)this.handler).getAvailableRecipeCount(); ++i) {
            ...
            int l = j / 4;
            int m = y + l * 18 + 2;
            this.client.getItemRenderer().renderInGuiWithOverrides(matrices, ((ChipRefineryRecipe)list.get(i)).getOutput(this.client.world.getRegistryManager()), k, m);
        }
